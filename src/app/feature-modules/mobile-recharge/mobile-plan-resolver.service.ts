...
export class MobilePlanResolverService implements Resolve<any> {
  ...
  resolve(): Observable<any> {
    const current_search = JSON.parse(
      sessionStorage.getItem('mobile_search') || '{}'
    );
    return this._mobileRechargeService.getPlanForMobileNo(
      current_search?.currentBillerId,
      current_search?.currentCircle
    );
  }
}
